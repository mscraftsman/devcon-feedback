<template>
  <div :class="{ 'px-3' : !nested }" class="listing--container">
    <v-toolbar color="transparent" class="listing-toolbar blue-grey darken-3 px-3" dark flat ml-0 v-if="!nested">
      ...
      <v-toolbar-title class="ml-0 pl-0">Visitor </v-toolbar-title>
      <v-spacer></v-spacer>
    ...
    <v-toolbar color="transparent" class="listing-toolbar" flat ml-0 v-else>
      ...
      <v-toolbar-title>Visitor </v-toolbar-title>
      <v-spacer></v-spacer>
      <v-btn mr-0 class="success" color="white" small flat @click="create">
        <v-icon dark>add</v-icon> Add
      </v-btn>
    ...
    <v-alert type="info" value="true" color="primary" outline icon="info" v-else-if="entities.length===0 && !loading">
      No Visitors found. Would you like to create one now?
          ...
          <tr :class="{'highlight': recentlySaved(props.item.Id)}" :key="props.item.Id">
            <td class="justify-center layout">
              ...
              <v-tooltip top v-else>
                <v-btn slot="activator" icon class="mx-0" :to="{name: 'visitors_edit', params:{ id: props.item.Id }}">
                  <v-icon>edit</v-icon>
                </v-btn>
                <span>Edit</span>
              </v-tooltip>
    ...
    >
      <v-card flat>
        <component :is="'VisitorsEdit'" :key="nestedForm.id" @closeform="nestedFormClose" :visible="nestedForm.visible" :filter_id="nestedForm.id" :nested="true" />
      </v-card>
    </v-dialog>
    <!-- !# Edit In Place -->
...
import { CreateVisitorRequest } from "@/services/service_admin_pb";
import { DeleteRequest } from "@/services/service_admin_pb";
import { mapGetters } from 'vuex';

import { Visitor, ListOpts, ListSortOpts } from "@/services/models_pb.js";
  ...
  data() {
    return {
      ...
      headers: [
        ...
        {text: "Bookmark", value: "Bookmark", sortable: false},
        {text: "Photo Link", value: "PhotoLink", sortable: false},
      ],
      entities: [],
      loading: false
    };
  ...
  methods: {    
      ...
      count(req) {
        return new Promise((resolve, reject) => {
          cli.countVisitors(req, (err, resp) => {
            if (err) {
              console.log(err);
      ...
      fetchEntities: function(req){
        return new Promise((resolve, reject) => {
          cli.listVisitors(req, (err, resp) => {
            ...
            this.loading = false;
            this.snackbarShow('Visitors Loaded');

            let entities = resp.getVisitorsList().map(entity => {
              return {
                Id: entity.getId(),
      ...
      duplicate(id) {
        ...
        cli.getVisitor(req, (err, resp) => {
          ...
          this.loading = false;
          ///
          toBeDuplicatedEntity = resp.getVisitor();
          this.create("existing", toBeDuplicatedEntity);
        });
      },
      create: function(from, existingEntity) {
        let req = new CreateVisitorRequest();
        req.setKey(this.token);

        if (from === "existing" && existingEntity) {
          req.setVisitor(existingEntity);
        } else if (this.nested) {
          let entity = new Visitor();
          // entity[this.rpc](this.filter_id);
          req.setVisitor(entity);
        }

        this.snackbarShow('Loading Visitors');
        this.loading = true;

        cli.createVisitor(req, (err, resp) => {
          if (err) {
            console.log(err);
          ...
          if (this.nested) {
            this.nestedForm = {
              visible: true,
              id: resp.getVisitor().getId(),
              label: ""
            };
          } else {
            this.$router.push({
              name: "visitors_edit",
              params: { id: resp.getVisitor().getId() }
            });
          }
        });
      },
      ...
      deleteConfirm: function() {
          ...
          cli.deleteVisitor(req, (err, resp) => {
            if (err) {
                console.log(err);
