<template>
  <div :class="{ 'px-3' : !nested }" class="listing--container">
    <v-toolbar color="transparent" class="listing-toolbar blue-grey darken-3 px-3" dark flat ml-0 v-if="!nested">
      ...
      <v-toolbar-title class="ml-0 pl-0">Feedback </v-toolbar-title>
      <v-spacer></v-spacer>
    ...
    <v-alert type="info" value="true" color="primary" outline icon="info" v-else-if="entities.length===0 && !loading">
      No Feedbacks found. Would you like to create one now?
          ...
          <tr :class="{'highlight': recentlySaved(props.item.Id)}" :key="props.item.Id">
            <td class="justify-center layout">
              ...
              <v-tooltip top v-else>
                <v-btn slot="activator" icon class="mx-0" :to="{name: 'feedbacks_edit', params:{ id: props.item.Id }}">
                  <v-icon>edit</v-icon>
                </v-btn>
                <span>Edit</span>
              </v-tooltip>
    ...
    >
      <v-card flat>
        <component :is="'FeedbacksEdit'" :key="nestedForm.id" @closeform="nestedFormClose" :visible="nestedForm.visible" :filter_id="nestedForm.id" :nested="true" />
      </v-card>
    </v-dialog>
    <!-- !# Edit In Place -->
...
import { CreateFeedbackRequest } from "@/services/service_admin_pb";
import { DeleteRequest } from "@/services/service_admin_pb";
import { mapGetters } from 'vuex';

import { Feedback, ListOpts, ListSortOpts } from "@/services/models_pb.js";
  ...
  data() {
    return {
      ...
      headers: [
        ...
        {text: "Reaction 1", value: "Reaction1", sortable: false},
        {text: "Reaction 2", value: "Reaction2", sortable: false},
        {text: "Reaction 3", value: "Reaction3", sortable: false},
        {text: "Reaction 4", value: "Reaction4", sortable: false},
      ],
      entities: [],
      loading: false
    };
  ...
  methods: {    
      ...
      count(req) {
        return new Promise((resolve, reject) => {
          cli.countFeedbacks(req, (err, resp) => {
            if (err) {
              console.log(err);
      ...
      fetchEntities: function(req){
        return new Promise((resolve, reject) => {
          cli.listFeedbacks(req, (err, resp) => {
            if (err) {
              console.log(err);
              this.snackbarShow(err.message, "error");
              return;
            ...
            this.loading = false;
            this.snackbarShow('Feedbacks Loaded');

            let entities = resp.getFeedbacksList().map(entity => {
              return {
                Id: entity.getId(),
                ...
                Reaction1: entity.getReaction1(),
                Reaction2: entity.getReaction2(),
                Reaction3: entity.getReaction3(),
                Reaction4: entity.getReaction4(),
      ...
      duplicate(id) {
        ...
        cli.getFeedback(req, (err, resp) => {
          ...
          this.loading = false;
          ///
          toBeDuplicatedEntity = resp.getFeedback();
          this.create("existing", toBeDuplicatedEntity);
        });
      },
      create: function(from, existingEntity) {
        let req = new CreateFeedbackRequest();
        req.setKey(this.token);

        if (from === "existing" && existingEntity) {
          req.setFeedback(existingEntity);
        } else if (this.nested) {
          let entity = new Feedback();
          // entity[this.rpc](this.filter_id);
          req.setFeedback(entity);
        }

        this.snackbarShow('Loading Feedbacks');
        this.loading = true;

        cli.createFeedback(req, (err, resp) => {
          if (err) {
            console.log(err);
          ...
          if (this.nested) {
            this.nestedForm = {
              visible: true,
              id: resp.getFeedback().getId(),
              label: ""
            };
          } else {
            this.$router.push({
              name: "feedbacks_edit",
              params: { id: resp.getFeedback().getId() }
            });
          }
        });
      },
      ...
      deleteConfirm: function() {
          ...
          cli.deleteFeedback(req, (err, resp) => {
            if (err) {
                console.log(err);
