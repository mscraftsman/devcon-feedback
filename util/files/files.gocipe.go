...
import (
	...
	"github.com/mscraftsman/devcon-feedback/util/imagist"
...
func Upload(fileName string, fileContent []byte, options *UploadOptions) (string, string, error) {
	...
	case TypeImage:
		if !filetype.IsImage(buf) {
			err = errors.New("Image expected")
		} else {
			err = _imagist.Add(buf, fileDiskPath, options.ImgDimensions, true)
		}
	case TypeVideo:
		if !filetype.IsVideo(buf) {
			err = errors.New("Video expected")
		}
	case TypeAudio:
		if !filetype.IsAudio(buf) {
			err = errors.New("Audio expected")
		}
	case TypeDocument, TypeSheet:
		if !filetype.IsDocument(buf) {
			err = errors.New("Document expected")
		}
	case TypePDF:
		if !matchers.Pdf(buf) {
			err = errors.New("PDF expected")
		}
	case TypeCSV:
		// TODO: check file mime, etc
		err = errors.New("Unsupported file uploaded")
	default:
		err = errors.New("Unsupported file uploaded")
	}
